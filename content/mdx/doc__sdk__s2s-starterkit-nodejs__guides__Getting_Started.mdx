...
description: ""
type: "sdk"
source: "https://developers.openrainbow.com/doc/sdk/s2s-starterkit-nodejs/guides/Getting_Started"
lastSynced: "2026-02-20T07:12:33.846Z"
---
# Documentation
...
[Rainbow Authentication Portal](https://hub.openrainbow.com/api/authentication/)

Authentication API

S2S Portal

Server to server API. For the moment it is essentially used for Instant messaging communication.
...
![S2S NodeJS Starter Kit composition](/docs/sdk/s2s-starterkit-nodejs/guides/./images/s2s-starterkit-zmq.jpg)

S2S NodeJS StarterKit implements S2S client APIs in NodeJS to interact with Rainbow and a local REST server (to handle webhook events via callback url). Rainbow acts as a client for the REST server (by POSTING events data on a public endpoint defined by the callbak url).
...
rainbow-s2s-starterkit-nodejs/config/StarterKitConfig.json.sample

starter kit main configuration file (to rename .json and update according to your Rainbow registration params). It defines bot credentials, localhost server settings.
...
Configures the Rainbow server host we are connecting to and tells Rainbow who we are

credentials

contains registered account credentials

application

contains the application registration keys appID and appSecret

logs

contains logs enablement config

im

enables or disables automatic IM read acknowledgement

rainbow\_notification\_service

configure each component (Rainbow-S2S-Starterkit-NodeJS internal proxy named ngrok or your own reverse proxy and zmq ) involved in webhook event notification
...
**rainbow\_onconnectioncreated**

Fired when the connection with rainbow is created bot not sign in

**rainbow\_onconnectionerror**

Fired when the connection can't be done with Rainbow (ie. issue on sign-in)

**rainbow\_onmessagereceived**

Fired when a one-to-one message is received

**rainbow\_onmessageserverreceiptreceived**

Fired when the message has been received by the server

**rainbow\_onmessagereceiptreceived**

Fired when the message has been received by the recipient

**rainbow\_onmessagereceiptreadreceived**

Fired when the message has been read by the recipient

**rainbow\_onallmessagereceiptreceived**

Fired when the all message has been received by the recipient

**rainbow\_onallmessagereceiptsent**

Fired when the all message has been sent to the recipient

**rainbow\_onpresencechanged**
...
**rainbow\_onbubbleremoved**

Fired when a bubble the connected user is member of is deleted

**rainbow\_onbubbleinvitationreceived**
...
Fired when the connection has stooped
...
-   [Getting started](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-getting-started-0)
-   [Prerequisites](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-prerequisites-29)
-   [What are S2S API modules used in Rainbow-S2S-StarterKit-NodeJS](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-what-are-s2s-api-modules-used-in-rainbow-s2s-starterkit-nodejs-61)
-   [Rainbow OpenAPI Specifications files (swagger files)](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-rainbow-openapi-specifications-files-swagger-files-73)
-   [Rainbow-S2S-StarterKit-NodeJS composition](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-rainbow-s2s-starterkit-nodejs-composition-115)
-   [Configuration file](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-configuration-file-127)
-   [Client API (Authentication and S2S portal API )](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-client-api-authentication-and-s2s-portal-api--218)
-   [Webhook event handler (REST server)](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-webhook-event-handler-rest-server-256)
-   [How to use Rainbow-S2S-StarterKit-NodeJS](/docs/sdk/s2s-starterkit-nodejs/guides/Getting_Started#name-how-to-use-rainbow-s2s-starterkit-nodejs-500)
